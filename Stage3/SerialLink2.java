...
//
    ...
    double[] e = new double[q.length];
    for (int i = 0; i < q.length; i++) {
      e[i] = qd[i] - q[i];
      if (e[i] > Math.PI) {
        e[i] -= 2 * Math.PI;
      } else if (e[i] < Math.PI * (-1)) {
        e[i] += 2 * Math.PI;
      }
      res[i] += Kp * e[i];
    }
...
//
    ...
    double[][] inverseJ = SerialLink2.inverseJacobian3(q);
    // 2.Calculate position error
    double[] e = new double[3];
    e[0] = xd - tool.x();
    e[1] = yd - tool.y();
    e[2] = (theta_d - tool.theta()) % Math.PI;
    ...
    //also use as reference https://dellaert.github.io/20S-3630/notes/3-manipulators.pdf
    for (int i = 0; i < res.length; i++) {
      res[i] += Kp * (inverseJ[i][0] * e[0] + inverseJ[i][1] * e[1] + inverseJ[i][2] * e[2]);
    }
