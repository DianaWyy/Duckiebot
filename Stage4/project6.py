# -*- coding: utf-8 -*-
"""Copy of Project 6 Student.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1jpB4QG8RekOK14GcWhonK-LlBl-PdDRN
...
def transform_cloud(bTa, clouda):
    ...
    """
    clouda = np.insert(clouda, 3, [1], axis=0)
    tclouda = np.matmul(bTa.matrix(), clouda)
    return tclouda[:-1]
...
def icp(clouda, cloudb, initial_transform=gtsam.Pose3(), max_iterations=25):
    ...
    while count < max_iterations:
      newclouda = transform_cloud(bTa, clouda)
      newcloudb = assign_closest_pairs(newclouda, cloudb)
      newbTa = estimate_transform(clouda, newcloudb)
      if bTa.equals(newbTa, 1e-2):
        icp_series.append([newclouda, cloudb])
        bTa = newbTa
        break
      bTa = newbTa
      icp_series.append([newclouda, cloudb])
...
def populate_factor_graph(graph, initial_estimates, initial_pose, clouds):
    ...
    for i in range(len(clouds) - 1):
        ...
        T, icpiters = icp(clouds[i], clouds[i+1], initial_transform=prev_T)
        # TODO: Set T to its inverse: use `gtsam.Pose3.inverse()`
        prev_T = T
        T = gtsam.Pose3.inverse(T)
    ...
    for i in range(0, len(clouds) - 2, 2):
        # TODO: Run ICP between clouds (hint: use inital_tranform argument)
        T, icpiters = icp(clouds[i], clouds[i+2], initial_transform=prev_T)
        # TODO: Set T to its inverse: use `gtsam.Pose3.inverse()`
        prev_T = T
        T = gtsam.Pose3.inverse(T)
        # TODO: Add a `gtsam.BetweenFactorPose3()` to the graph
